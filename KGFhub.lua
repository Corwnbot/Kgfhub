...
task.spawn(function()
  while task.wait() do
    if block and block.Parent == workspace then
      if _G.AutoFarmNearest
      or _G.TeleportToIsland
      or _G.AutoFinishTrial
      or _G.AutoWoodPlanks
      or _G.AutoFarm_Level
      -- or _G.AutoGift
      -- or _G.AutoCandy
      or _G.AutoFarmSea
      or _G.AutoEliteHunter
      or _G.AutoPiratesSea
      or _G.AutoFarmBossSelected
      or _G.AutoRengoku
      or _G.TeleportToFruit
      or _G.AutoFactory
      or _G.AutoBartiloQuest
      or _G.AutoFarmRaid
      or _G.AutoRaceV2
      or _G.AutoCakePrince
      or _G.AutoDoughKing
      or _G.RipIndraLegendaryHaki
      or _G.AutoRipIndra
      or _G.AutoUnlockSaber
      or _G.AutoSawBoss
      or _G.AutoEnelBossPole
      or _G.AutoMusketeerHat
      or _G.AutoKenV2
      or _G.AutoFarmKen
      or _G.AutoFarmBone
      or _G.AutoCursedCaptain
      or _G.AutoFarmEctoplasm
      or _G.AutoKitsuneIsland
      or _G.AutoSoulReaper
      or _G.AutoSoulGuitar
      or _G.TeleportToMirage
      or _G.AutoSecondSea
      or _G.AutoThirdSea
      or _G.AutoDarkbeard
      or _G.AutoKillLawBoss
      or _G.AutoChestTween
      or _G.AutoRainbowHaki
      or _G.AutoFarmMaterial
      or _G.AutoSharkmanKarate
      or _G.AutoKillDonSwan
      or _G.AutoSoulGuitar
      or _G.AutoCursedDualKatana
      or _G.AutoDeathStep
      or _G.AutoDragonTalon
      or _G.AutoElectricClaw
      or _G.AutoSuperhuman
      or _G.AutoMasteryFightingStyle
      or _G.AutoGodHuman
      or _G.AutoTushita
      or _G.AutoFarmMastery
      or _G.NPCtween
      or _G.KillAllBosses then
        _G.OnFarm = true
      else
        _G.OnFarm = false
      end
...
local function PlayerTP(Tween_Pos)
  ...
  elseif block then
    if Distance <= 450 then
      ...
      TweenInfo.new(Distance / tonumber(_G.TweenSpeed * 1.8), Enum.EasingStyle.Linear),
...
local function BoatTP(Boat, pos)
  if Boat then
    ...
    TweenInfo.new(Distance / _G.SeaBoatSpeed, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
    {CFrame = pos}):Play()
  end
end
...
local function AutoKillAura()
  while _G.AutoKillAura do task.wait()
    KillAura()
  end
end
  ...
  if pos then
    repeat task.wait()
      for _,v in pairs(pos) do
        ...
        if block then
          local tween = game:GetService("TweenService"):Create(block,
          TweenInfo.new((block.Position - v.p).Magnitude / _G.TweenSpeed, Enum.EasingStyle.Linear),
          {CFrame = v})tween:Play()tween.Completed:Wait()
        end
      end
    until not _G.AutoFarm_Level or Enemies:FindFirstChild(NPC or Quest[3])
    -- or _G.TimeToGift < 90
    return
  end
end
...
local function EspFlowers()
  while _G.EspFlowers do task.wait()
...
local function EspFruits()
  while _G.EspFruits do task.wait()
...
local function EspIslands()
  while _G.EspIslands do task.wait()
...
local function EspChests()
  while _G.EspChests do task.wait()
...
local function AttackDistance()
  while _G.AttackDistance do task.wait()
    pcall(function()
...
local function PlayerClick()
  ...
  if _G.AutoClick and (tick() - time) >= _G.AutoClickDelay then
    task.spawn(function()
...
local function ActiveHaki()
  ...
  if _G.AutoHaki and not VerifyHaki then
    FireRemote("Buso")
  end
end
    ...
    if self.Name == "RemoteEvent" and getnamecallmethod():lower() == "fireserver" then
      if typeof(Vector) == "Vector3" then
        if _G.AimbotPos then
          if _G.AutoFarmSea or _G.AutoWoodPlanks then
            if _G.SeaAimBotSkill then
              return oldhook(self, _G.AimbotPos, ...)
            end
          elseif _G.AutoFarmMastery then
...
local function EquipTool()
  local backpack = Player.Backpack
  for _,tool in pairs(backpack:GetChildren()) do
    if tool and tool.ToolTip == _G.FarmTool then
      EquipToolName(tool.Name)return
    end
  end
end
...
local function AutoChestBypass()
  ...
  while _G.AutoChestBypass do task.wait()
...
local function AutoChestTween()
  while _G.AutoChestTween do task.wait()
...
local function VerifyRaidPirate()
  return _G.AutoPiratesSea and (tick() - PirateTime) <= 10
end

local function VerifyEliteBoss()
  return _G.AutoEliteHunter and VerifyNPC("Urban")
  or _G.AutoEliteHunter and VerifyNPC("Deandre")
  or _G.AutoEliteHunter and VerifyNPC("Diablo")
end
...
local function Configure(Farm)
  if Farm == "Level" then
    ...
    elseif _G.TeleportToFruit and FruitFind() then return true
    elseif _G.AutoSecondSea and PlayerLevel.Value >= 700 then return true
    elseif _G.AutoThirdSea and PlayerLevel.Value >= 1500 then return true
    elseif VerifyNPC("Awakened Ice Admiral") and _G.AutoRengoku then return true
    elseif VerifyNPC("Darkbeard") and _G.AutoDarkbeard then return true
    elseif VerifyNPC("rip_indra True Form") and _G.AutoRipIndra then return true
    elseif VerifyTool("God's Chalice") and _G.AutoRipIndra then return true
    elseif VerifyNPC("Cake Prince") and _G.AutoCakePrince then return true
    elseif VerifyNPC("Dough King") and _G.AutoCakePrince then return true
    elseif VerifyNPC("Don Swan") and _G.AutoKillDonSwan then return true
    elseif VerifyNPC("Soul Reaper") and _G.AutoSoulReaper then return true
    elseif VerifyTool("Hallow Essence") and _G.AutoSoulReaper then return true
    end
  elseif Farm == "Raid Pirate" then
    if not VerifyRaidPirate() and _G.AutoFarmBone then return true
    elseif not VerifyRaidPirate() and _G.AutoFarm_Level then return true
    elseif not VerifyRaidPirate() and _G.AutoEliteHunter then return true
    elseif not VerifyRaidPirate() and _G.TeleportToFruit then return true
    end
  elseif Farm == "All Bosses" then
    if _G.KillAllBosses and VerifyBosses() then
    end
  ...
  elseif Farm == "Elite Hunter" then
    if _G.TeleportToFruit and FruitFind() then return true
    elseif VerifyRaidPirate() then return true
    elseif not VerifyEliteBoss() and _G.AutoCandy then return true
    elseif _G.AutoFarm_Level and not VerifyEliteBoss() then return true
    elseif _G.AutoFarmBone and not VerifyEliteBoss() then return true
    elseif VerifyNPC("rip_indra True Form") and _G.AutoRipIndra then return true
    elseif VerifyNPC("Cake Prince") and _G.AutoCakePrince then return true
    elseif VerifyNPC("Dough King") and _G.AutoCakePrince then return true
    elseif VerifyNPC("Soul Reaper") and _G.AutoSoulReaper then return true
  ...
  elseif Farm == "Fruit" then
    if _G.AutoSecondSea and PlayerLevel.Value >= 700 then return true
    elseif VerifyRaidPirate() then return true
    elseif _G.AutoFarmRaid and TimerG.Visible then return true
    elseif VerifyNPC("Awakened Ice Admiral") and _G.AutoRengoku then return true
    elseif VerifyNPC("Darkbeard") and _G.AutoDarkbeard then return true
    elseif VerifyNPC("Soul Reaper") and _G.AutoSoulReaper then return true
    elseif VerifyTool("Hallow Essence") and _G.AutoSoulReaper then return true
  ...
  elseif Farm == "Bone" then
    if _G.AutoFarm_Level then return true
    elseif VerifyRaidPirate() then return true
  ...
  elseif Farm == "Ectoplasm" then
    if _G.TeleportToFruit and FruitFind() then return true
    elseif _G.AutoCursedCaptain and VerifyNPC("Cursed Captain") then return true
    end
  elseif Farm == "Hallow Boss" then
    if _G.AutoFarmBone and not VerifyNPC("Soul Reaper") and not VerifyTool("Hallow Essence") then return true
    elseif _G.AutoRipIndra and VerifyNPC("rip_indra True Form") then
    end
  elseif Farm == "Raid" then
    if _G.TeleportToFruit and FruitFind() and not TimerG.Visible then return true
    end
  elseif Farm == "Don Swan" then
    if _G.AutoFarm_Level and not VerifyNPC("Don Swan") then return true
    elseif _G.AutoFarmEctoplasm and not VerifyNPC("Don Swan") then return true
    elseif _G.AutoDarkbeard and VerifyNPC("Darkbeard") then return true
    elseif _G.AutoCursedCaptain and VerifyNPC("Cursed Captain") then return true
    end
  ...
  elseif Farm == "Cake Prince" then
    ...
    elseif not VerifyNPC("Cake Prince") and _G.TeleportToFruit and FruitFind() then return true
    elseif not VerifyNPC("Dough King") and _G.TeleportToFruit and FruitFind() then return true
    elseif VerifyNPC("Soul Reaper") and _G.AutoSoulReaper then return true
    elseif VerifyNPC("rip_indra True Form") and _G.AutoRipIndra then return true
    elseif VerifyTool("God's Chalice") and _G.AutoRipIndra then return true
    end
  elseif Farm == "Rip Indra" then
    if not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form") and _G.AutoFarm_Level then return true
    elseif not VerifyNPC("rip_indra True Form") and _G.RipIndraLegendaryHaki and GetButton() then
    elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form") and _G.AutoEliteHunter then return true
    elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form") and _G.TeleportToFruit then return true
    elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form") and _G.AutoFarmBone then return true
    elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form") and _G.AutoCakePrince then return true
    elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form") and _G.AutoCakePrince then return true
    end
  end
end
...
local function AutoFarm_Level()
  while _G.AutoFarm_Level do task.wait()
    if Configure("Level") then
      ...
      elseif Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
        PlayerTP(Enemie.HumanoidRootPart.CFrame + _G.FarmPos)
...
local function AutoFarmMastery()
  ...
  while _G.AutoFarmMastery do task.wait()
    local SkillHealth = _G.HealthSkill
    local Tool = _G.ToolMastery
    ...
    elseif Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
      ...
      if Health and Health < Enemie.Humanoid.MaxHealth * SkillHealth / 100 then
        PlayerTP(Enemie.HumanoidRootPart.CFrame + Vector3.new(0, _G.FarmDistance, 0))
...
local function AutoKitsuneIsland()
  ...
  while _G.AutoKitsuneIsland do task.wait()
    local plrChar = Player.Character
    local Boat = GetBoat()
...
local function AutoFarmSea()
  ...
  task.spawn(function()
    ...
    task.spawn(function()
      while _G.AutoFarmSea do task.wait()
    ...
    while _G.AutoFarmSea do task.wait()
      if _G.SeaLevelTP == "inf" then
        Distance = -100000000
      elseif _G.SeaLevelTP == "6" then
        Distance = -42700
      elseif _G.SeaLevelTP == "5" then
        Distance = -38000
      elseif _G.SeaLevelTP == "4" then
        Distance = -34000
      elseif _G.SeaLevelTP == "3" then
        Distance = -30000
      elseif _G.SeaLevelTP == "2" then
        Distance = -26000
      elseif _G.SeaLevelTP == "1" then
        Distance = -22000
      end
    end
  end)
  ...
  task.spawn(function()
    while _G.AutoFarmSea do task.wait()
      ...
      if plrH then
        if plrH.Health < plrH.MaxHealth * _G.HealthPanic / 100 then
          InPanicMode = true
          ...
          until not _G.AutoFarmSea
          or not _G.PanicMode
          or not Boat
          or not plrChar
          or not plrH
          or plrH.Health > plrH.MaxHealth * math.clamp(_G.HealthPanic, 20, 75) / 100
  ...
  while _G.AutoFarmSea do task.wait()
    ...
    if _G.PanicMode and InPanicMode and Boat then
    elseif not Boat then
      local BuyBoatDistance = (plrChar.PrimaryPart.Position - Vector3.new(-6123, 16, -2247)).Magnitude
      if BuyBoatDistance <= 5 then
        FireRemote("BuyBoat", "Guardian")
    ...
    elseif _G.Terrorshark and Terrorshark and Terrorshark:FindFirstChild("HumanoidRootPart") then
    ...
    elseif _G.Piranha and Piranha and Piranha:FindFirstChild("HumanoidRootPart") then
      
      PlayerTP(Piranha.HumanoidRootPart.CFrame + _G.FarmPos)
      pcall(function()PlayerClick()ActiveHaki()EquipTool()BringNPC(Piranha)plrChar.Humanoid.Sit = false end)
      
    elseif _G.FishCrewMember and FishCrewMember and FishCrewMember:FindFirstChild("HumanoidRootPart") then
      
      PlayerTP(FishCrewMember.HumanoidRootPart.CFrame + _G.FarmPos)
      pcall(function()PlayerClick()ActiveHaki()EquipTool()BringNPC(FishCrewMember)plrChar.Humanoid.Sit = false end)
      
    elseif _G.Shark and Shark and Shark:FindFirstChild("HumanoidRootPart") then
      
      PlayerTP(Shark.HumanoidRootPart.CFrame + _G.FarmPos)
      pcall(function()PlayerClick()ActiveHaki()EquipTool()plrChar.Humanoid.Sit = false end)
    ...
    elseif _G.PirateGrandBrigade and Boat1 and Boat1.PrimaryPart then
      
      PlayerTP(Boat1.PrimaryPart.CFrame + _G.FarmPos)
    ...
    elseif _G.PirateBrigade and Boat2 and Boat2.PrimaryPart then
      
      PlayerTP(Boat2.PrimaryPart.CFrame + _G.FarmPos)
      pcall(function()ActiveHaki()plrChar.Humanoid.Sit = false end)
    ...
    elseif _G.FishBoat and Boat3 and Boat3.PrimaryPart then
      
      PlayerTP(Boat3.PrimaryPart.CFrame + _G.FarmPos)
      pcall(function()ActiveHaki()plrChar.Humanoid.Sit = false end)
...
local function AutoEliteHunter()
  while _G.AutoEliteHunter do task.wait()
    if Configure("Elite Hunter") then
      ...
      if EliteBoss and EliteBoss:FindFirstChild("HumanoidRootPart") then
        PlayerTP(EliteBoss.HumanoidRootPart.CFrame + _G.FarmPos)
...
local function AutoKenV2()
  ...
  task.spawn(function()
    while _G.AutoKenV2 do task.wait()
  ...
  while _G.AutoKenV2 do task.wait()
    ...
    elseif not BossProgress then
      ...
      elseif CaptainElephant and CaptainElephant:FindFirstChild("HumanoidRootPart") then
        PlayerTP(CaptainElephant.HumanoidRootPart.CFrame + _G.FarmPos)
        pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
      else
...
local function AutoMusketeerHat()
  ...
  task.spawn(function()
    while _G.AutoMusketeerHat do task.wait()
  ...
  while _G.AutoMusketeerHat do task.wait()
    ...
    elseif not BossProgress then
      ...
      elseif CursedCaptain and CursedCaptain:FindFirstChild("HumanoidRootPart") then
        PlayerTP(CursedCaptain.HumanoidRootPart.CFrame + _G.FarmPos)
        pcall(function()PlayerClick()ActiveHaki()EquipTool()BringNPC(Enemies1)end)
      else
        PlayerTP(CFrame.new(-13393, 319, -8423))
      end
...
local function AutoCursedDualKatana()
  ...
  -- Final Quest
  task.spawn(function()
    while _G.AutoCursedDualKatana do task.wait()
      if FinalQuest then
  ...
  task.spawn(function()
    ...
    while _G.AutoCursedDualKatana do task.wait(0.5)
...
local function AutoRainbowHaki()
  while _G.AutoRainbowHaki do task.wait()
...
local function AutoSoulGuitar()
  ...
  while _G.AutoSoulGuitar do task.wait()
...
local function AutoUnlockSaber()
  ...
  task.spawn(function()
    while _G.AutoUnlockSaber do task.wait()
...
local function AutoFarmEctoplasm()
  while _G.AutoFarmEctoplasm do task.wait()
...
local function AutoFarmMaterial()
  ...
  task.spawn(function()
    while _G.AutoFarmMaterial do task.wait()
      local Material = _G.MaterialSelected
      if Sea1 then
...
local function AutoFarmBone()
  while _G.AutoFarmBone do task.wait()
...
local function AutoPiratesSea()
  while _G.AutoPiratesSea do task.wait()
...
local function AutoSecondSea()
  ...
  while _G.AutoSecondSea do task.wait()
    if PlayerLevel.Value < 700 then
      ...
      elseif VerifyNPC("Ice Admiral") and IceMap:FindFirstChild("Door") and not IceMap.Door.CanCollide and IceMap.Door.Transparency > 0 then
        ...
        if IceAdmiral and IceAdmiral:FindFirstChild("HumanoidRootPart") then
          PlayerTP(IceAdmiral.HumanoidRootPart.CFrame + _G.FarmPos)
          pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
        end
      else
...
local function AutoThirdSea()
  ...
  task.spawn(function()
    while _G.AutoThirdSea do task.wait()
  ...
  while _G.AutoThirdSea do task.wait()
    if PlayerLevel.Value < 1500 then
      ...
      if RipIndra and RipIndra:FindFirstChild("HumanoidRootPart") then
        PlayerTP(RipIndra.HumanoidRootPart.CFrame + _G.FarmPos)
        pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
      elseif QuestProgress == 0 then
        if boss and boss:FindFirstChild("HumanoidRootPart") then
          PlayerTP(boss.HumanoidRootPart.CFrame + _G.FarmPos)
          pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
        else
          local plrChar = Player and Player.Character and Player.Character.PrimaryPart
...
local function AutoBartiloQuest()
  ...
  task.spawn(function()
    while _G.AutoBartiloQuest do task.wait()
    ...
    if PlayerLevel.Value >= 850 then
      ...
      elseif BartiloQuestLevel == 1 then
        ...
        if Enemie1 and Enemie1:FindFirstChild("HumanoidRootPart") then
          PlayerTP(Enemie1.HumanoidRootPart.CFrame + _G.FarmPos)
...
local function AutoRipIndra()
  while _G.AutoRipIndra do task.wait()
    if Configure("Rip Indra") then
      ...
      if RipIndra1 and RipIndra1:FindFirstChild("HumanoidRootPart") then
        PlayerTP(RipIndra1.HumanoidRootPart.CFrame + _G.FarmPos)
        pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
...
local function AutoKillDonSwan()
  while _G.AutoKillDonSwan do task.wait()
...
local function AutoCursedCaptain()
  while _G.AutoCursedCaptain do task.wait()
    if _G.AutoFarm_Level and not VerifyNPC("Cursed Captain") then
    elseif _G.AutoFarmEctoplasm and not VerifyNPC("Cursed Captain") then
    else
...
local function AutoCakePrince()
  task.spawn(function()
    while _G.AutoCakePrince do task.wait(1)
      FireRemote("CakePrinceSpawner")
    end
  end)
  
  while _G.AutoCakePrince do task.wait()
    if Configure("Cake Prince") then
      ...
      if VerifyNPC("Dough King") then
        ...
        if DoughKing and DoughKing:FindFirstChild("HumanoidRootPart") then
          PlayerTP(DoughKing.HumanoidRootPart.CFrame + _G.FarmPos)
          pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
        end
      elseif VerifyNPC("Cake Prince") then
        ...
        if CakePrince and CakePrince:FindFirstChild("HumanoidRootPart") then
          PlayerTP(CakePrince.HumanoidRootPart.CFrame + _G.FarmPos)
          pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
        end
      else
        if not _G.AutoFarm_Level and not _G.AutoFarmBone then
...
local function AutoDoughKing()
  local CocoaStats = ""
  task.spawn(function()
    while _G.AutoDoughKing do task.wait()
...
local function KillAllBosses()
  ...
  task.spawn(function()
    while _G.KillAllBosses do task.wait()
      local BossTable = {}
  ...
  while _G.KillAllBosses do task.wait()
    if SelectedBoss then
      ...
      if Boss and GetQuest and _G.TakeQuestBoss and not QuestVisible() and not VerifyQuest(Boss.Name) then
...
local function AutoFarmBossSelected()
  local GetQuest, GetQuestPos, Quest, NotBossPos, BossName = false, nil, nil, nil, _G.BossSelected or ""
  local QuestActive = Player.PlayerGui.Main.Quest
  
  while _G.AutoFarmBossSelected do task.wait()
    BossName = _G.BossSelected or ""
    ...
    if plrChar then
      ...
      if VerifyNPC(BossName) then
        ...
        if GetQuest and _G.TakeQuestBoss and not QuestActive.Visible and not string.find(QuestActive.Container.QuestTitle.Title.Text, BossName) then
          StartQuest1(Quest, QuestNun or 3)
        elseif Boss1 and Boss1:FindFirstChild("HumanoidRootPart") then
          PlayerTP(Boss1.HumanoidRootPart.CFrame + _G.FarmPos)
...
local function AutoSoulReaper()
  while _G.AutoSoulReaper do task.wait()
    if Configure("Hallow Boss") then
      ...
      if SoulReaper and SoulReaper:FindFirstChild("HumanoidRootPart") then
        PlayerTP(SoulReaper.HumanoidRootPart.CFrame + _G.FarmPos)
        pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
...
local function AutoFarmNearest()
  ...
  while _G.AutoFarmNearest do task.wait()
    if _G.AutoFarm_Level or _G.AutoRipIndra or _G.AutoCakePrince then
    else
...
local function AutoRaceV2()
  local QuestProgress = 0
  task.spawn(function()
    while _G.AutoRaceV2 do task.wait()
...
local function AutoRengoku()
  while _G.AutoRengoku do task.wait()
    if VerifyNPC("Darkbeard") and _G.AutoDarkbeard then
    else
      ...
      elseif VerifyNPC("Awakened Ice Admiral") then
        ...
        if AwakenedIceAdmiral and AwakenedIceAdmiral:FindFirstChild("HumanoidRootPart") then
          PlayerTP(AwakenedIceAdmiral.HumanoidRootPart.CFrame + _G.FarmPos)
          pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
        end
      else
...
if Sea3 then
  ...
  AutoSea:AddToggle("Toggle", {Title = "Auto Kitsune Island", Callback = function(Value)
    _G.AutoKitsuneIsland = Value;AutoKitsuneIsland()
  end})
  AutoSea:AddToggle("Toggle", {Title = "Auto Trade Azure Ember",Callback = function(Value)
    ...
    task.spawn(function()
      ...
      while _G.TradeAzureEmber do task.wait(1)
  ...
  AutoSea:AddToggle("Toggle", {Title = "Auto Farm Sea",Callback = function(Value)
    _G.AutoFarmSea = Value;AutoFarmSea()
  end})
  ...
  AutoSea:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        while _G.AutoWoodPlanks do task.wait()
  ...
  AutoSea:AddToggle("Toggle", {Title = "Teleport To Shark Hunter",Callback = function(Value)
    _G.NPCtween = Value
    task.spawn(function()
      while _G.NPCtween do task.wait()
...
if Sea3 and Player.UserId == 2764978820 then
  ...
  MirageTab:AddToggle("Toggle", {Title = "Auto Finish Trial", Callback = function(Value)
    ...
    task.spawn(function()
      local PlayerRace
      task.spawn(function()
        while _G.AutoFinishTrial do task.wait()
...
if PlayerLevel.Value < 2551 then
  ...
  local function AutoStats()
    ...
    while _G.AutoStats do task.wait()
      if Melee then
        AddStats("Melee")
      end
      if Defense then
...
if PlayerLevel.Value >= 2550 and Sea3 then
  MainFarm:AddToggle("Toggle", {Title = "Auto Farm Candy", Callback = function(Value)
    ...
    task.spawn(function()
      ...
      while _G.AutoCandy do task.wait()
...
MainFarm:AddToggle("Toggle", {Title = "Auto Gift", Callback = function(Value)
  ...
  task.spawn(function()
    ...
    while _G.AutoGift do task.wait()
...
FruitAndRaid:AddToggle("Toggle", {
  ...
  Callback = function(Value)
    ...
    task.spawn(function()
      while _G.TeleportToFruit do task.wait()
...
FruitAndRaid:AddToggle("Toggle", {
  ...
  Callback = function(Value)
    ...
    task.spawn(function()
      while _G.AutoRandomFruit do task.wait(1)
...
elseif Sea2 or Sea3 then
  ...
  FruitAndRaid:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        task.spawn(function()
          while _G.AutoFarmRaid do task.wait(0.5)
        ...
        task.spawn(function()
          while _G.AutoFarmRaid do task.wait()
            if Configure("Raid") then
            else
  ...
  FruitAndRaid:AddToggle("Toggle", {Title = "Auto Buy Chip", Callback = function(Value)
    ...
    task.spawn(function()
      while _G.AutoBuyChip do task.wait()
        if not VerifyTool("Special Microchip") then
          FireRemote("RaidsNpc", "Select", _G.SelectRaidChip)task.wait(1)
        end
      end
    end)
  end})
...
elseif Sea2 then
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        while _G.AutoLegendarySword do task.wait()
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        while _G.AutoBuyLawChip do task.wait()
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        while _G.AutoStartLawRaid do task.wait()
...
elseif Sea3 then
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        while _G.AutoCollectYama do task.wait()
          pcall(function()
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        while _G.AutoTushita do task.wait()
  ...
  local ActiveButtonToggle = QuestsTabs:AddToggle("Toggle", {Title = "Auto Active Button Haki Color", Callback = function(Value)
    ...
    task.spawn(function()
      while _G.RipIndraLegendaryHaki do task.wait()
        local plrChar = Player and Player.Character and Player.Character.PrimaryPart
    ...
    task.spawn(function()
      while _G.RipIndraLegendaryHaki do task.wait()
        if not _G.AutoFarm_Level and not _G.AutoFarmBone and not _G.AutoCakePrince then
          if GetButton() then
            PlayerTP(GetButton().CFrame)
          elseif not GetButton() and not _G.AutoRipIndra then
            PlayerTP(CFrame.new(-5119, 315, -2964))
          end
        end
      end
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        while _G.AutoDeathStep do task.wait()
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        while _G.AutoElectricClaw do task.wait()
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        task.spawn(function()
          while _G.AutoSharkmanKarate do task.wait()
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        task.spawn(function()
          while _G.AutoDragonTalon do task.wait()
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        while _G.AutoSuperhuman do task.wait()
          if VerifyTool("Black Leg") then
            MasteryBlackLeg = GetToolLevel("Black Leg")
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        while _G.AutoGodHuman do task.wait()
          if Sea2 then
            FireRemote("TravelZou")
          end
  ...
  QuestsTabs:AddToggle("Toggle", {
    ...
    Callback = function(Value)
      ...
      task.spawn(function()
        ...
        while _G.AutoMasteryFightingStyle do task.wait()
          local MaxMastery = _G.AutoMasteryValue
...
QuestsTabs:AddToggle("Toggle", {
  ...
  Callback = function(Value)
    ...
    task.spawn(function()
      while _G.AutoBuyHakiColor do task.wait(0.5)
        pcall(function()
...
Teleport:AddToggle("Toggle", {Title = "Teleport To Island",Callback = function(Value)
  ...
  task.spawn(function()
    while _G.TeleportToIsland do task.wait()
...
Misc:AddSlider("Slider", {Title = "Farm Distance",Min = 5,Max = 50,Default = 20,Rounding = 1,Callback = function(Value)
  _G.FarmPos = Vector3.new(0, Value or 15, Value or 10)_G.FarmDistance = Value
end})
...
Misc:AddToggle("Toggle", {
  ...
  Callback = function(Value)
    ...
    task.spawn(function()
      ...
      while _G.WalkOnWater do task.wait(0.1)
